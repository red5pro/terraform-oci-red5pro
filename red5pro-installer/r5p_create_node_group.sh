...
create_jwT_token() {
    ...
    for i in {1..5}; do
        JWT_TOKEN=$(curl --insecure -s -X 'PUT' "$SM_URL/as/v1/auth/login" -H 'accept: application/json' -H "Authorization: Basic $USER_AND_PASSWORD_IN_BASE64")
        JWT_TOKEN=$(jq -r '.token' <<<"$JWT_TOKEN")

        if [ -z "$JWT_TOKEN" ] || [ "$JWT_TOKEN" == "null" ]; then
            log_w "JWT token was not created! - Attempt $i"
            log_d "JWT_TOKEN: $JWT_TOKEN"
        else
